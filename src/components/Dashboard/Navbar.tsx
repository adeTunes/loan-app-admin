                  ...
                  <ul className="navChildrenList">
                    {item.children.map((el, idx) => (
                      <li key={idx} className="eachList">
                        <span style={{
                          background: selected === el.title ? "#39cdcd17" : "none",
                          borderLeft: selected === el.title ? "3px solid #39CDCC" : "3px solid white"
                        }} onClick={() => setSelected(el.title)} className="listGroup">
                          <img src={el.icon} alt="" />
                          <span>{el.title}</span>
                        </span>
                      </li>
